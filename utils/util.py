...
def adjust_learning_rate(optimizer, epoch, base_lr, decay_every=30):
    """每 decay_every 个 epoch 衰减一次学习率"""
    lr = base_lr * (0.1 ** (epoch // decay_every))
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
    return lr
...
def create_save_path(opt):
    ...
    predict_test_path = os.path.join(save_path, 'predict_test')
    predict_train_path = os.path.join(save_path, 'predict_train')
    predict_val_path = os.path.join(save_path, 'predict_val')

    check_dir(save_path), check_dir(save_path), check_dir(log_path), check_dir(checkpoint_path), \
...
def create_data_path(opt):
    ...
    train_txt_path = os.path.join(data_inform_path, 'train.txt')
    val_txt_path = os.path.join(data_inform_path, 'val.txt')
    test_txt_path = os.path.join(data_inform_path, 'test.txt')
...
def get_mean_std(flag):    
    #5m    
    if flag == 'FUSAR':
        means = [59.69668691, 59.69668691, 59.69668691]
        std = [45.07595097, 45.07595097, 45.07595097]
    
    #3m
    elif flag == 'GF3_3m':
        means = [60.37033299, 60.37033299, 60.37033299]
        std = [43.08863746, 43.08863746, 43.08863746]
    
    elif flag == 'MSLCC':
        means = [94.02087474, 94.02087474, 94.02087474]
        std = [37.02579262, 37.02579262, 37.02579262]
